          ...
          */
          ...
          if (!isset($address) || strlen($address) < 5 ||  strlen($address) > 50) {
           ...
           elseif (empty($info)) {
               ...
               $emailExist = checkEmailUser($email);// cette variable stock l'utilisateur qui possède l'email renseigner en argument dans la fonction checkEmailUser 
               //si l'email n'existe pas dans la Bdd la variable stock false
               

               if ($emailExist) {
                    $info = alert("Cette adresse mail est deja utiliser" , "danger");
               }
               ...
               elseif (empty($info)) {
                    ...
                    $info = alert("Vous êtes inscrit Vous pouvez vous connecter.","success");
               }
           }
